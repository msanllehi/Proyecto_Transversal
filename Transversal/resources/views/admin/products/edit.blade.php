...
<div class="container mx-auto mt-32 mb-16 flex justify-center">
    <div class="w-full max-w-lg">
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-gray-700 font-bold mb-2">Nombre</label>
                <input type="text" name="name" id="name" class="w-full border rounded px-3 py-2 bg-orange-500" value="{{ $product->name }}" required>
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-700 font-bold mb-2">Descripción</label>
                <textarea name="description" id="description" class="w-full border rounded px-3 py-2 bg-orange-500" rows="4">{{ $product->description }}</textarea>
            </div>
            <div class="mb-4">
                <label for="price" class="block text-gray-700 font-bold mb-2">Precio (€)</label>
                <input type="number" name="price" id="price" class="w-full border rounded px-3 py-2 bg-orange-500" step="0.01" value="{{ $product->price }}" required>
            </div>
            <div class="mb-4">
                <label for="image" class="block text-gray-700 font-bold mb-2">URL de la imagen</label>
                <input type="text" name="image" id="image" class="w-full border rounded px-3 py-2 bg-orange-500" value="{{ $product->image }}" placeholder="https://example.com/image.jpg">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="subcategory_id" id="subcategory_id" class="w-full border rounded px-3 py-2 bg-orange-500" {{ !$product->category_id ? 'disabled' : '' }}>
            ...
            @if($product->image)
            <div class="mb-4">
                <p class="block text-gray-700 font-bold mb-2">Imagen actual:</p>
                <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-full h-40 object-cover rounded bg-orange-500">
            </div>
            @endif
